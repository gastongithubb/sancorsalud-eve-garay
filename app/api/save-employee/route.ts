...
import { insertEmployeeMetric, EmployeeMetricInsert } from '@/utils/database';

export async function POST(request: NextRequest) {
  console.log('API route hit: /api/save-employee');
  try {
    const employeesData: EmployeeMetricInsert[] = await request.json();
    ...
    for (const employeeData of employeesData) {
      ...
      await insertEmployeeMetric(employeeData);
    }
